...
<title>Dealer's Returns - Summary</title>
<style type="text/css">
* {
	margin: 0;
	padding: 0;
...
<script type="text/javascript" src="js/dealer-return.js"></script>
<script type="text/javascript">
$(function() {

var m = 'modules/dealer-return.php';
var f = function() {
	$('#search').button();
	$('#search').click(function() { filterReturnDealer(); });
	...
	success: function(data, status) {
			 ...
			 $('input#fsup').jsonSuggest({data: suggestList.supplierlist, minCharacters: 2, onSelect: function(item){ $('#fhsup').val(item.id); }});            
			 }
	});
	...
	success: function(data, status) {
			...
			$('input#fdealer').jsonSuggest({data: suggestList.dealerlist, minCharacters: 2, onSelect: function(item){
				$('#fdealer').val(item.dealer);	
			}});				 
			}
	});	
