...
<title>Manual Remittances</title>
<style type="text/css">
img {
	border: 0;
}
...
a.tooltip-min span {display:none; padding:2px 3px; margin-top:27px; margin-left:-25px; width:100px; font-size:.9em; text-align:center;}
...
<script type="text/javascript" src="js/manual-remittances.js"></script>
<script type="text/javascript">
$(function() {

var m = 'modules/manual-remittances.php';
var f = function() {
	$('#add').button();
	$('#edit').button();
	$('#delete').button();
	$('#cutoff').button();
	$('#search').button();	
	$('#add').click(function() { manual_remittance(1); });
	$( "#fs" ).datepicker({
		showOn: "button",
	...
	$('#edit').click(function() { manual_remittance(2); });
	$('#delete').click(function() { confirmMRDelete(); });
	$('#search').click(function() { filterManualRemittance(); });
	$('#cutoff').click(function() { confirmCutoff(); });

/* 	$('#add-deduction').button();
	$('#edit-deduction').button();
	$('#delete-deduction').button();
	
	$('#add-deduction').click(function() { deductionM(1); });
	$('#edit-deduction').click(function() { deductionM(2); });
	$('#delete-deduction').click(function() { confirmMDeductionDelete(); }); */
	...
	$.ajax({
		type: 'get',
		url: 'manual-remittances-ajax.php?p=check_cutoff',
		success: function(data, status) {
			if (parseInt(data) > 0) {
				$('#dco').html('End of the day');
				$('#hco').val(2);
			}
		}
	});
...
var dfs = new Date();
var fs = (dfs.getMonth() + 1) + dfs.getDate() + dfs.getFullYear();
content(9,0,'&fs=' + fs + '&fe=' + '&fcustomer=');
