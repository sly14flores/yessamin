...
var m = 'modules/deposits.php';
var f = function() {
	$('#add').button();
	$('#edit').button();
	$('#delete').button();
	$('#search').button();	
	$('#add').click(function() { deposit(1); });
	$( "#fs" ).datepicker({
		showOn: "button",
	...
	$('#edit').click(function() { deposit(2); });
	$('#delete').click(function() { confirmDDelete(); });
	$('#search').click(function() { filterDeposits(); });
	
	$.ajax({
		url: 'deposits-ajax.php?p=select_bank_account',
		type: 'get',
		success: function(data, status) {
			$('#faccount-name').html('<option value="0">All</option>' + data);
		}
	});	
